...
int smithy()
{
  ...
  // BUG: original line - if (supplyCount(estate, state) > 0){
  // BUG: the supplyCount parameter variables are switched around
  if (supplyCount(state, estate) > 0){
...
int great_hall()
{
      ...
      //+1 Actions
      // BUG: ORIGINAL LINE - state->numActions++;
      // BUG: the variable numActions is going down rather than up
      //state->numActions--;i
      //BUG FIX:
      state->numActions++;
...
int sea_hag()
{
  ...
  // BUG: the if statement works in a completely opposite intention
  if (i == currentPlayer){
